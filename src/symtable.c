...
/* Append a symbol to `s`, growing the array if necessary.
   Returns a pointer to the newly added slot. */
static Symbol *scope_push(Scope *s, const char *name, const char *type, int offset)
{
    if (s->count == s->cap) {
    ...
    if (!sym->name || !sym->type) die("out of memory in symtable_define");
    return sym;
}
